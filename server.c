...
int main(int argc, char *argv[])
{
	...
	if (send(clntSockfd, "Connected!!!\n", 
        ...
        while(1) {
            ...
            //****Jonathan: new line inserted
	         char newStr[numBytes];
            recvBuff[numBytes] = '\0';    
            strcpy(newStr, recvBuff);
            /* send data to the client ******Changed by Jonathan to prohibit buffer overflow*/
            if (send(clntSockfd, newStr, strlen(newStr), 0) == -1) {
                perror("send failed");
                close(clntSockfd);
                exit(1);
        }
